...
- `models.py` - **асинхронный слой хранения данных** (async SQLAlchemy: engine, сессии, модели `User` и `Advertisement`)
- `aiohttp_app.py` - базовая версия (Задание 1)
- `aiohttp_app_with_auth.py` - расширенная версия с авторизацией (Задание 2)
- `requirements.txt` - зависимости проекта
- `Dockerfile` - конфигурация Docker
- `docker-compose.yml` - конфигурация Docker Compose (API + PostgreSQL)

## 🗄️ Асинхронное хранилище данных

Данные (пользователи и объявления) хранятся в базе данных и обрабатываются
**полностью асинхронно** через `SQLAlchemy` в async-режиме (`create_async_engine`,
`async_sessionmaker`, `AsyncSession`). Синхронные словари больше не используются.

- **Локальный запуск:** по умолчанию используется SQLite через `aiosqlite`
  (`sqlite+aiosqlite:///./app.db`) — дополнительная настройка не требуется.
- **Docker / продакшен:** через переменную окружения `DB_DSN` подключается
  PostgreSQL через асинхронный драйвер `asyncpg`
  (`postgresql+asyncpg://app:app@db:5432/ads`).

Пароли хэшируются через **bcrypt**, токены авторизации выдаются в формате **JWT**
(Bearer). Таблицы создаются автоматически при старте приложения (`on_startup`).
...
**Или с помощью docker-compose (поднимает PostgreSQL + API):**
```bash
# Базовая версия на порту 8080 (+ PostgreSQL)
docker-compose up aiohttp-basic

# Версия с авторизацией на порту 8081 (+ PostgreSQL)
docker-compose up aiohttp-auth
```

Переменные окружения:

- `DB_DSN` — строка подключения к БД (async). По умолчанию SQLite; в docker-compose — PostgreSQL/asyncpg.
- `JWT_SECRET` — секрет для подписи JWT.
- `JWT_EXPIRES_HOURS` — время жизни токена в часах (по умолчанию 24).
...
> `password_hash` хранится в БД (bcrypt), но никогда не возвращается в ответах API.
